...
bool collides(const AxisAlignedBoundingBox &one, const AxisAlignedBoundingBox &two) {
    ...
    if (!oneHasDimension || !twoHasDimension) return true;  // Collide if the box has no dimensions

    bool oneIsInTwo = checkIfBoxContainsCorner(one, two);
    bool twoIsInOne = checkIfBoxContainsCorner(two, one);
    if (oneIsInTwo || twoIsInOne) return true;              // Collide if there is a corner that overlaps
    ...
    return (oneOverlapsTwo || twoOverlapsOne);              // Collide if boxes hover over each other
}
